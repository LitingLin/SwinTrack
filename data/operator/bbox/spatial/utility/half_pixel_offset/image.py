...
def get_image_center_point(image_size):
    return data.operator.bbox.spatial.utility.image.get_image_center_point(image_size, PixelCoordinateSystem.HalfPixelOffset)


def get_image_bounding_box(image_size):
    return data.operator.bbox.spatial.utility.image.get_image_bounding_box(image_size, PixelCoordinateSystem.HalfPixelOffset)
...
def bounding_box_is_in_image_boundary(bbox, image_size):
    image_bounding_box = get_image_bounding_box(image_size)
    return bbox[0] >= image_bounding_box[0] and bbox[1] >= image_bounding_box[1] and bbox[2] <= image_bounding_box[2] and bbox[3] <= image_bounding_box[3]
